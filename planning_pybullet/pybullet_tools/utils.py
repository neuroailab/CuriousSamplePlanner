...
def add_fixed_constraint_2(body, robot):
    body_link = BASE_LINK
    body_pose = get_pose(body)
    ...
    #                          parentFrameOrientation=unit_quat(),
    #                          childFrameOrientation=quat)
    constraint = p.createConstraint(robot, -1, body, -1,  # Both seem to work
                                    p.JOINT_FIXED, jointAxis=unit_point(),
                                    parentFramePosition=point,
                                    childFramePosition=unit_point(),
                                    parentFrameOrientation=quat,
...
def inverse_kinematics_helper(robot, link, target_pose, null_space=None):
    ...
    if null_space is not None:
        ...
        kinematic_conf = p.calculateInverseKinematics(robot, link, target_point, target_quat,
                                                      ...
                                                      physicsClientId=CLIENT, solver = p.IK_SDLS)
...
def is_pose_close(pose, target_pose, pos_tolerance=1e-5, ori_tolerance=1e-5*np.pi):
    (point, quat) = pose
    (target_point, target_quat) = target_pose
...
def inverse_kinematics(robot, link, target_pose, max_iterations=20, custom_limits={}, **kwargs):
    movable_joints = get_movable_joints(robot)
    for iterations in range(max_iterations):
        # TODO: stop is no progress
        # TODO: stop if collision or invalid joint limits
        kinematic_conf = inverse_kinematics_helper(robot, link, target_pose, null_space = get_null_space(robot, movable_joints))
