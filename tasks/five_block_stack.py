...
class FiveBlocks(Environment):
    ...
    def check_goal_state(self, config):
        ...
        vals.sort()
        if (vals[0] < 0.06 and (vals[1] > 0.06 and vals[1] < 0.16) and (vals[2] > 0.16 and vals[2] < 0.26) and (
                vals[3] > 0.26 and vals[3] < 0.36) and (vals[4] > 0.36 and vals[4] < 0.46)):
            return True
        return False
    ...
    def get_start_state(self):
        ...
        while collision:
            pos1, pos2, pos3, pos4, pos5 = self.reachable_pos(z=0), self.reachable_pos(z=0), self.reachable_pos(
                z=0), self.reachable_pos(z=0), self.reachable_pos(z=0)
