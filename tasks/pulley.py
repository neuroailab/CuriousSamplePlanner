...
class PulleySeesaw(Environment):
	...
	def check_goal_state(self, config):
		...
		tddist = lambda a, b, c:  math.sqrt((a-keystone[0])**2+(b-keystone[1])**2+(c-keystone[2])**2)<0.5
		in_bucket = int(tddist(config[0], config[1], config[2])) + int(tddist(config[6], config[7], config[8])) + int(tddist(config[12], config[13], config[14])) + tddist(config[18], config[19], config[20]) + int(tddist(config[24], config[25], config[26]))
		# if(in_bucket>0):
			# print(str(in_bucket) +" in bucket")
		if(tddist(config[0], config[1], config[2]) and tddist(config[6], config[7], config[8]) and tddist(config[12], config[13], config[14]) and tddist(config[18], config[19], config[20]) and tddist(config[24], config[25], config[26])):
			print(config[-2])
