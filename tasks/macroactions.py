...
from planning_pybullet.pybullet_tools.kuka_primitives import Detach, ApplyForce
from planning_pybullet.pybullet_tools.utils import WorldSaver, Euler
from planning_pybullet.pybullet_tools.utils import multiply, invert, set_pose, get_movable_joints, \
    set_joint_positions, add_fixed_constraint_2, end_effector_from_body, approach_from_grasp, Pose, Point, \
    inverse_kinematics, get_sample_fn, \
    get_joint_positions
...
class MacroAction:
    ...
    def reparameterize(self, block_to_move, pos):
        ...
        teleport_pose = Pose(Point(x=r * math.cos(theta), y=r * math.sin(theta), z=min(height, 1)),
                             Euler(roll=orig_euler[0], pitch=orig_euler[1], yaw=yaw))
    ...
    def execute(self, embedding, config, sim=False):
		...
		"""
        total_selectors = sum(
            [self.macroaction_list[macro_idx].num_selectors for macro_idx in range(len(self.macroaction_list))])
        ...
        if isinstance(self.macroaction_list[macroaction_index], PickPlace):
            ...
            mask_start = total_selectors + sum(
                [self.macroaction_list[macro_idx].num_params for macro_idx in range(macroaction_index)])
            ...
            start_index = len(self.objects) + sum(
                [self.macroaction_list[macroaction_index].object_params[i] for i in range(object_index)])
            ...
            (feasible, planning_commands, _) = self.macroaction_list[macroaction_index].execute(block_to_move,
                                                                                                teleport_pose, sim)
            ...
            (feasible, planning_commands, link_status) = self.macroaction_list[macroaction_index].execute(
...
class PickPlace(MacroAction):
    ...
    def feasibility_check(self, block_to_move, goal_pose, sim=False):
        ...
        ik_fn = get_ik_fn(self.robot, fixed=self.fixed,
                          teleport=self.teleport)  # These are functions which generate sequences of actions
        ...
        for grasp_gen in [get_grasp_gen(self.robot, direction) for direction in
                          ['top', 'side']]:  # Can put in other angles here, leaving as top for now
...
class AddLink(MacroAction):
    ...
    def execute(self, embedding, link_status, sim=False):
        ...
        assert object1_index * len(self.objects) + object2_index == object_pair_index or object2_index * len(
            self.objects) + object1_index == object_pair_index
        # Not already linked
