...
def nonholo_rrt(start, goal_sample, distance, sample, extend, collision, goal_test=lambda q: False, iterations=RRT_ITERATIONS, goal_probability=.2):
    if collision(start):
        return None
    ...
    for i in irange(iterations):
        ...
        col, gen = extend(last.config, s)
        if(not col):
            for q, collision in gen:
                if collision:
                    break
                last = TreeNode(q, parent=last)
                nodes.append(last)
                print(distance(q, goal_sample()))
                if(distance(q, goal_sample()) < 0.1):
