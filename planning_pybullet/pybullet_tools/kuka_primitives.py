...
class Command(object):
    ...
    def execute(self, time_step=0.0, hook=None):
        for i, body_path in enumerate(self.body_paths):
            for j in body_path.iterator():
                if(hook!=None):
                    hook()
...
def get_ik_fn(robot, fixed=[], teleport=False, num_attempts=100):
    ...
    def fn(body, pose, grasp):
        ...
        for _ in range(num_attempts):
            ...
            # Inverse kinematics: you give it an end effector and a target point in 3d space and it returns the joint angles
            ...
            if teleport:
                ...
                path = plan_direct_joint_motion(robot, conf.joints, q_grasp)
                if path is None:
...
def get_holding_motion_gen(robot, fixed=[], teleport=False, self_collisions=True):
    def fn(conf1, conf2, body, grasp, fluents=[]):
        ...
        if teleport:
            ...
            if path is None:
                print("plan joint motion failed")
