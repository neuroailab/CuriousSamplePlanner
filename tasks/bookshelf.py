...
import h5py
import imageio
from planning_pybullet.pybullet_tools.kuka_primitives import BodyPose, BodyConf, Command, \
    get_grasp_gen, \
    get_ik_fn, get_free_motion_gen, get_holding_motion_gen
from planning_pybullet.pybullet_tools.utils import WorldSaver, enable_gravity, connect, dump_world, \
    set_pose, \
...
from planning_pybullet.pybullet_tools.utils import multiply, invert, get_pose, set_pose, \
    get_movable_joints, \
    ...
    control_joints, interpolate_poses
...
class BookShelf(Environment):
    ...
    def get_start_state(self):
        ...
        while collision:
            conf = np.array(
                self.reachable_pos(z=z) + [0, 0, random.uniform(-math.pi, math.pi)] + self.reachable_pos(z=z) + [0, 0,
                                                                                                                 random.uniform(
                                                                                                                     -math.pi,
                                                                                                                     math.pi)] + self.book_pos + self.book_rot + self.macroaction.link_status)
            self.set_state(conf)
