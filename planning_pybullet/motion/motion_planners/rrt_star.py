...
def nonholo_rrt_star(start, goal, distance, sample, extend, collision, radius, max_time=INF, max_iterations=INF, goal_probability=.2, informed=True):
    ...
    while (t0 - time()) < max_time and it < max_iterations:
        ...
        if informed and goal_n is not None and distance(start, s) + distance(s, goal) >= goal_n.cost:
            print("Intermediate cose:"+str(distance(start, s)+distance(s, goal)))
            print("Goal cost:"+str(goal_n.cost))
            continue
        if it % 100 == 0:
        ...
        col, path = extend(nearest.config, s)
        if(col):
            continue
        path = safe_path(path, collision)
        ...
        # if safe and do_goal:
        print(distance(new.config, goal))
        if do_goal and distance(new.config, goal) < 0.1:
            goal_n = new
        ...
        for n in neighbors:
            ...
            if n.cost + d < new.cost:
                col, dpath = extend(n.config, new.config)
                if(not col):
                    path = safe_path(dpath, collision)
                    if len(path) != 0 and distance(new.config, path[-1]) < 0.1:
        ...
        for n in neighbors:  # TODO - avoid repeating work
            ...
            if new.cost + d < n.cost:
                dpath = extend(new.config, n.config)
                if(not coll):
                    path = safe_path(dpath, collision)
                    if len(path) != 0 and distance(n.config, path[-1]) < 0.1:
