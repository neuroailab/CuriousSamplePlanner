...
class MacroAction():
	def __init__(self, macroaction_list=[], experiment_dict = None):
		self.experiment_dict = experiment_dict
		self.reachable_max_height = self.experiment_dict['reachable_max_height']
		self.max_reach_horiz = self.experiment_dict['max_reach_horiz']
		self.min_reach_horiz = self.experiment_dict['min_reach_horiz']
	...
	def feasibility_check(self, block1, block2, sim=False):
		...
		if(block1 != block2 and check_state_collision(block1, block2)):
			...
			if result2 is None:
				return (None, False)
			path1, = result2
			return Command(path1.body_paths+[link_object]), True
