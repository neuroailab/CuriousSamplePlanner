...
class DDPGLearningPolicy(EnvPolicy):
	def __init__(self, *args):
		super(DDPGLearningPolicy, self).__init__(*args)
		self.AC_MULT = 5
	...
	def select_action(self, obs):
		obs = opt_cuda(obs).type(torch.FloatTensor)
		action = self.agent.select_action(obs, self.ounoise, self.param_noise)*self.AC_MULT
		info = {}
		return action[0], info

	def store_results(self, next_state, action, reward, action_infos, prev_state, done, goal):
		mask = opt_cuda(torch.Tensor([True]))
		reward = opt_cuda(torch.Tensor([reward]))
		parent_config = opt_cuda(prev_state.type(torch.FloatTensor))
	...
	def update(self, total_numsteps):
		...
		return np.mean(np.array(value_losses)), np.mean(np.array(policy_losses)), 0
