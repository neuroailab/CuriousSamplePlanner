...
class Environment():
    ...
    def compute_reward(self, obs, goal, uk):
        ...
        return np.array([int(self.check_goal_state(obs[i])) for i in range(obs.shape[0])])
    ...
    def run_until_stable(self, hook=None, dt=0.001):
        ...
        while (True):
            ...
            if (dist(config_t, config) < 1e-4 or t>time_limit):
                break
    ...
    def step(self, action, terminate_unreachable=False, state_estimation=False):
        ...
        if(state_estimation):
            pics = []
            for yaw, pit in self.perspectives:
                pic, _, _ = take_picture(yaw, pit, 0)
                pics.append(torch.tensor(pic).type(torch.FloatTensor).permute(2, 0, 1))
            inputs = opt_cuda(torch.unsqueeze(torch.cat(pics), dim=0))
        else:
