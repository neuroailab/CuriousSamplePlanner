...
def retrace(visited, q):
    if q is None:
        return []
    return retrace(visited, visited[q].parent) + [q]
...
def astar_modified(start, goal, distance, neighbors, max_iterations=INF):  # TODO - put start and goal in neighbors
    if collision(start) or collision(goal):
        return None
    queue = [(0, start)]
    visited = {tuple(start): Node(0, None)}
    ...
    while len(queue) != 0 and iterations < max_iterations:
        ...
        for next in neighbors(current):
            ...
            if (tuple(next) not in visited or next_g < visited[tuple(next)].g):
                visited[tuple(next)] = Node(next_g, current)
                # ValueError: The truth value of an array with more than one
                # element is ambiguous.
                heappush(queue, (next_g, next))
    return None
...
def astar(start, goal, distance, neighbors, collision,
    ...
    while len(queue) != 0 and iterations < max_iterations:
        ...
        for next in neighbors[current]:
            next_g = current_g + distance(current, next)
            if (next not in visited or next_g < visited[next].g) and not collision(next):
                visited[next] = Node(next_g, current)
                # ValueError: The truth value of an array with more than one
                # element is ambiguous.
                heappush(queue, (next_g+distance(next, goal)+random.uniform(0, 0.1), next_g, next))
    return None
